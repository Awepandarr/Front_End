    ...
    <div class="bg-white rounded-lg shadow-lg p-6">
      ...
      <div v-else class="max-h-80 overflow-y-auto mb-6">
        <div 
          ...
          :key="index"
          class="flex flex-col md:flex-row md:justify-between md:items-center py-4 border-b border-gray-200 last:border-0"
        >
          <div class="flex-1">
            <div class="flex justify-between md:block">
              <h3 class="text-lg font-medium">{{ item.name }}</h3>
              <span class="md:hidden text-lg font-bold">${{ (item.price * item.quantity).toFixed(2) }}</span>
            </div>
            ...
            <div class="flex items-center mt-2">
              <button 
                ...
                class="bg-gray-200 hover:bg-gray-300 px-3 py-1 rounded-l text-lg focus:outline-none"
              ...
              <button 
                ...
                class="bg-gray-200 hover:bg-gray-300 px-3 py-1 rounded-r text-lg focus:outline-none"
              >
                +
              </button>
      ...
      <div class="mt-6" v-if="items.length > 0">
        ...
        <div class="grid grid-cols-2 gap-3 mb-4">
          <button 
            @click="checkout('CASH')" 
            class="bg-green-500 hover:bg-green-600 text-white py-3 rounded-lg flex items-center justify-center space-x-2"
          >
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <button 
            @click="checkout('CARD')" 
            class="bg-blue-500 hover:bg-blue-600 text-white py-3 rounded-lg flex items-center justify-center space-x-2"
          >
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="flex space-x-3">
          <button 
            @click="holdOrder" 
            class="flex-1 bg-yellow-500 hover:bg-yellow-600 text-white py-3 rounded-lg"
          >
            Hold Order
          </button>
          <button 
            @click="cancelOrder" 
            class="flex-1 bg-red-500 hover:bg-red-600 text-white py-3 rounded-lg"
          >
            Cancel
          </button>
        </div>
  ...
  export default {
    ...
    setup(props, { emit }) {
      ...
      const holdOrder = () => {
        ...
        // In a real implementation, you would save this order to a held orders database
