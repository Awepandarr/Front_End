<template>
  <div class="bg-white shadow-lg rounded-lg overflow-hidden">
    <div class="p-8 bg-gradient-to-r from-blue-500 to-blue-600 text-white">
      ...
      <p class="text-lg">View historical sales data and reports</p>
    </div>
          ...
          <div class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              ...
              <div class="bg-green-500 text-white p-6 rounded-lg shadow">
                ...
                <div class="mt-2 text-4xl font-bold">${{ formatCurrency(selectedReport.total_payments) }}</div>
              </div>
  ...
  setup() {
    ...
    const createSalesChart = () => {
      try {
        ...
        console.log('Chart element exists:', !!salesChart.value);
        console.log('Reports length:', reports.value.length);
        ...
        // Use our reports data
        // Sort data by date
        const sortedReports = [...reports.value].sort((a, b) => new Date(a.date) - new Date(b.date));

        // Log the sorted data to verify it's correct
        console.log('Sorted reports for chart:', sortedReports);
        ...
        // Log prepared data
        console.log('Chart data:', { labels, salesData, paymentsData, discountData });
    ...
    const fetchReportHistory = async () => {
      ...
      try {
        console.log('Fetching reports for date range:', startDate.value, 'to', endDate.value);
        
        // Use the report service to fetch data
        ...
        console.log('Reports received:', reports.value.length);
        
        // If no reports are returned and in development, use sample data
        if (reports.value.length === 0) {
          console.log('No reports received, using sample data');
          reports.value = [
        ...
        // After data is loaded, create the chart with a delay to ensure DOM is updated
        nextTick(() => {
          ...
          setTimeout(() => {
            if (salesChart.value && reports.value.length > 0) {
              ...
              console.warn('Cannot create chart - missing canvas reference or no data');
              console.log('Chart ref:', !!salesChart.value);
              console.log('Reports length:', reports.value.length);
            }
          }, 100);
        });
    ...
    // Initialize chart on component mounted
    onMounted(() => {
      console.log('Component mounted, fetching initial reports');
      fetchReportHistory();
    ...
    const parsePaymentBreakdown = (breakdownString) => {
      if (!breakdownString || breakdownString === 'No payment data available') {
        ...
        <body>
          ...
          <div class="summary">
            ...
            <div class="summary-card payments">
              ...
              <div class="value">${formatCurrency(report.total_payments)}</div>
            </div>
